...
struct EventLoop* eventLoopInitEx(const char* threadName)
{
    ...
    evLoop->isQuit = false; //开始的时候还没有运行，也就不需要退出
    evLoop->threadID = pthread_self();//获取当前线程的id
    pthread_mutex_init(&evLoop->mutex, NULL); //互斥锁的初始化
    strcpy(evLoop->threadName, threadName == NULL ? "MainThread" : threadName);  //在c语言里数组是不能不同等号复制的，要做一个拷贝
...
int eventLoopRun(struct EventLoop* evLoop)
{
    ...
    // 比较线程ID是否正常  判断当前的线程id和保存的线程id是否相等
...
int eventLoopAddTask(struct EventLoop* evLoop, struct Channel* channel, int type)
{
    ...
    */
    if (evLoop->threadID == pthread_self())
    {
        // 当前子线程(基于子线程的角度分析)  基于主线程的角度分析 也有进入到这里的情况
        eventLoopProcessTask(evLoop);
    }
    else
    {
