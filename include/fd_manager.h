...
{
    /**
     * @brief 文件句柄上下文类 （存储文件描述符fd的信息）
     * @details 管理文件句柄(fd)类型(是否socket)
     *          是否阻塞,是否关闭,读/写超时时间
     */
    ...
    private:
        ...
        bool m_isSocket : 1;     // 文件句柄是否为socket
        bool m_sysNonblock : 1;  // 系统是否设置为非阻塞
        bool m_userNonblock : 1; // 用户是否设置为非阻塞
